...
(function(){
    ...
    var MarkerClusterer =  
        ...
        BMapLib.MarkerClusterer = function(map, options){
            ...
            this._map.addEventListener("moveend",function(){
				if(opts["lockClus"] && opts["lockClus"] == true){
            ...
            if(opts["onmouseover"]!=null && isFunction(opts["onmouseover"])){
            	this._onmouseover = opts["onmouseover"];
            }
            
			if(opts["onmouseout"]!=null && isFunction(opts["onmouseout"])){
    ...
    MarkerClusterer.prototype._redraw = function () {
        ...
        if(this._onmouseover){ 
        	var mouseoverFunc = this._onmouseover;
            for(var i=0,cluster;cluster=this._clusters[i];i++){//遍历所有的聚合点对象
                cluster._clusterMarker.onmouseover=function(){//在每个聚合点对象的视图对象上绑定事件
                    //这样就可以为每一个聚合点绑定点击事件了
                	mouseoverFunc(this);
                }
            }
		...
		if(this._onmouseout){  
        	var mouseoutFunc = this._onmouseout;
            for(var i=0,cluster;cluster=this._clusters[i];i++){//遍历所有的聚合点对象
                cluster._clusterMarker.onmouseout=function(){//在每个聚合点对象的视图对象上绑定事件
                    //这样就可以为每一个聚合点绑定点击事件了
                	mouseoutFunc(this);
                }
            }
    ...
    function Cluster(markerClusterer){
		...
		this._data = markerClusterer.getData();
        this._clusterMarker = new BMapLib.TextIconOverlay(this._center, this._markers.length, {"styles":this._markerClusterer.getStyles()},this._data);
...
//        this.updateClusterMarker();
    ...
    Cluster.prototype.updateClusterMarker = function () {
		...
		if(this._data != undefined){
			this._clusterMarker.setText(this._markers.length+ '</br>' +this._data);
		}else{
			this._clusterMarker.setText(this._markers.length);
		}
