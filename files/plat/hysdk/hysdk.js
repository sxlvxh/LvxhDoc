...
HY_SDK_WS.prototype.initWebRtc = function (opts) {
	var _this  = this;
...
HY_SDK_WS.prototype.startCapture = function (opts) {
	var _this  = this;
    //var server = opts.server;//"wss://192.168.2.176:8989";
	this.hyWebRtc.startCapture(opts);
   		
} 

HY_SDK_WS.prototype.playUrl = function (opts) {
	var _this  = this;
    //var server = opts.server;//"wss://192.168.2.176:8989";
	this.hyWebRtc.playAttach(opts);
...
mainPlayer.prototype.initControlAllBar = function()
{
	...
	root.onmousemove = function(ev){
	   ...
	   //console.log("!!!!!!!!"+ "clientHeight::"+root.clientHeight+"|||top::"+tTop+" ev.clientY:::"+ev.clientY);
		if(root.clientHeight-(ev.clientY-tTop) <= 32){
			bar.style.display = 'block';
		}else{
			if(_this.isSettingVoice){
			}else{
				bar.style.display = 'none';
			}
		}
...
//          	console.log('监听的DOM', entry.target)
        	...
        	if(entry.contentRect.width == (wh-2)){
        		clickImg.src = 'imgs/fullscreen_exit.png';
        		_this.full = 2;
        	}else{
        		clickImg.src = 'imgs/fullscreen.png';
        		_this.full = 1;
        	}
        })
      });
...
{
	  var _this = this;
	  var p = {width:"68%",height:"68%",pnode:_this.opts.pnode, mainPlay: _this}; 
	 _this.players.push(new hyVideo(p));

	  for(var i= 0;i<2;i++)
	  {
    	  var p1 = {width:"32%",height:"34%",pnode:_this.opts.pnode, mainPlay: _this}; 
    	 _this.players.push(new hyVideo(p1));
	  }
	  
	   for(var i= 0;i<2;i++)
	  {
    	  var p1 = {width:"34%",height:"32%",pnode:_this.opts.pnode, mainPlay: _this}; 
    	 _this.players.push(new hyVideo(p1));
	  }
	  
   	 var p1 = {width:"32%",height:"32%",pnode:_this.opts.pnode, mainPlay: _this}; 
	 _this.players.push(new hyVideo(p1));
};
mainPlayer.prototype.setLayout7 = function()
{
	  var _this = this;
	  var p = {width:"75%",height:"66%",pnode:_this.opts.pnode, mainPlay: _this}; 
	 _this.players.push(new hyVideo(p));
	  for(var i= 0;i<2;i++)
	  {
    	  var p1 = {width:"25%",height:"33%",pnode:_this.opts.pnode, mainPlay: _this}; 
    	 _this.players.push(new hyVideo(p1));
	  }
...
mainPlayer.prototype.setVideoParam = function(activeHyVideo, opts){
	try{
		...
		activeHyVideo.setAvailableBtn({obj: activeHyVideo});
...
mainPlayer.prototype.setLayoutModel = function(){
	this.layoutModel = {
	    ...
	    6:{shapeNum: 2,layShapeRelate:{0:0,1:1,2:1,3:2,4:2,5:3},0: {width : "68%","height" :"68%"},1: {width : "32%","height" :"34%"},2: {width : "34%","height" :"32%"},3: {width : "32%","height" :"32%"}},
	    ...
	    10:{shapeNum: 1,0: {width : 100/3+"%","height":"25%"}},
     	11:{shapeNum: 1,0: {width : 100/3+"%","height":"25%"}},
	    12:{shapeNum: 1,0: {width : 100/3+"%","height":"25%"}},
	    13:{shapeNum: 1,0: {width : "25%","height":"25%"}},
	    14:{shapeNum: 1,0: {width : "25%","height":"25%"}},
	    15:{shapeNum: 1,0: {width : "25%","height":"25%"}},
	    16:{shapeNum: 1,0: {width : "25%","height":"25%"}},
     	17:{shapeNum: 1,0: {width : "25%","height":"20%"}},
      	18:{shapeNum: 1,0: {width : "25%","height":"20%"}},
   		19:{shapeNum: 1,0: {width : "25%","height":"20%"}},
        20:{shapeNum: 1,0: {width : "25%","height":"20%"}},
        21:{shapeNum: 1,0: {width : "25%","height":100/6+"%"}},
        22:{shapeNum: 1,0: {width : "25%","height":100/6+"%"}},
        23:{shapeNum: 1,0: {width : "25%","height":100/6+"%"}},
...
//	  	console.log("Y:::"+ev.clientY+"  pt||::"+videoPanel.offsetParent.offsetTop+" top::"+videoPanel.offsetTop+"|||||"+tTop);
//	  	console.log(ev.clientY-(videoPanel.offsetParent.offsetTop+videoPanel.offsetTop));
	  	if(ev.clientY-tTop<=32){
		  	_this.toolBar.style.display = 'block';
	  	}else{
	  		if(_this.isSettingVoice){
			}else{
...
hyVideo.prototype.initSoundPandel = function(param){
	 ...
	 //声音控制版面
	  ...
	  _this.soundControl = soundControl;
	  ...
	  if(param.type && param.type=="allControl"){
	  	 soundUp.style.transform = 'rotate(180deg)';
	  	 soundUp.style.top = "120px";
	  	 soundControl.style.top='';
	  	 soundControl.style.bottom='40px';
	  }
	  ...
	  _this.toolBar.appendChild(soundControl);
}
...
HY_MEETING.prototype.starCaptureOrPlayUrl = function(){
   ...
   if(device.deviceType && device.deviceType == 1){//使用的是ipc
   	var opts={};
...
HY_MEETING.prototype.notifyMeetingStatusInfo = function(json){
 	...
 	}catch(e){console.error("notifyMeetingStatusInfo:::"+e)}
};
...
HYCanvas.prototype.drawCenterCircle= function(){
 ...
 function hyWebRtc(opts)
 {
	...
	if(hySdk && hySdk.opts.log && hySdk.opts.log === true)
	{
		this.logLevel = "all";
	}
 }
 hyWebRtc.prototype.init = function()
 {
	...
	Janus.init({debug: _this.logLevel, callback: function(){
			_this.janus = new Janus({
							server: _this.server,
							success: function() {
								_this.hySdk.log(" new Janus success : " + _this.isConnected());
							},
...
hyWebRtc.prototype.isConnected = function()
  ...
  hyWebRtc.prototype.startCapture = function(opts)
 {
 	...
 	var map = {
 	   ...
 	   send :function(body)
 	   {
 	   	  _this.hySdk.log(" send message to janus opaqueId : " +map.opaqueId, body);
 	   	  map.pluginHandle.send(body)
 	   },
 	   videoNode:opts.videoNode,
 	   media:{data: true },
	...
	_this.janus.attach(
		{
			...
			success: function(pluginHandle) {
				...
				map.pluginHandle.createOffer(
    			{
    				media: map.media,	// ... let's negotiate data channels as well
    				simulcast: false,
    				success: function(jsep) {
    					 _this.hySdk.log("hyWebRtc.prototype.startCapture ! attach jsep : ", jsep);
    				...
    				error: function(error) {
    					 _this.hySdk.log("  hyWebRtc.prototype.startCapture attach error...", error);
    				}
    			});
			},
			error: function(error) {
			...
			consentDialog: function(on) {
				...
				if(on) {
					$.blockUI({
						message: '<div><img src="imgs/up_arrow.png"/></div>',
						css: {
							border: 'none',
							padding: '15px',
			...
			slowLink: function(uplink, lost) {
				 _this.hySdk.log("Janus reports problems " + (uplink ? "sending" : "receiving") +
					" packets on this PeerConnection (" + lost + " lost packets)");
			},
			onmessage: function(msg, jsep) {
				_this.hySdk.log(" =============== Got a message :::"+ jsep.type);
				if(jsep) {
				   if(jsep.type == "answer"){
			...
			onlocalstream: function(stream) {
					...
					if(!videoTracks || videoTracks.length === 0) {
						// No webcam
						 _this.hySdk.log(" onlocalstream No webcam");
					} else {
					}
 ...
 hyWebRtc.prototype.playAttach = function(opts)
 {
 	...
 	var map = {
 	   ...
 	   send :function(body)
 	   {
 	   	  _this.hySdk.log("playAttach  send message to janus opaqueId : " +map.opaqueId, body);
 	   	  map.pluginHandle.send(body)
 	   },
 	   videoNode:opts.videoNode,
	...
	_this.janus.attach(
		{
			...
			success: function(pluginHandle) {
				map.pluginHandle = pluginHandle;
				var body = { request: "play", uuid: "5425","server_ip":'192.168.2.198',"url":map.playUrl};
			...
			onmessage: function(msg, jsep) {
				_this.hySdk.log("hyWebRtc.prototype.playAttach Got a message :::"+ jsep.type);
				if(jsep) {
				    if(jsep.type == "offer"){
						_this.hySdk.log("Handling SDP as well...", jsep);
