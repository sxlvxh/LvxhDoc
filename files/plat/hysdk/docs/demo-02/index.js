			...
			if(msg.msgType == "kickout"){
               ...
               //  const {type} = msg;
                ...
                switch(type) {
					...
					console.log(1);
                       // handleReceiveOffer(cont);
					   // remotePeerConnection.setRemoteDescription(new RTCSessionDescription(cont.payload.sdp))
                  
						//、.catch(setSessionDescriptionError);
	
						createdAnswer(cont.payload.sdp);
						//attachMediaIfReady();
                        break;
                    case SIGNALING_ANSWER:
					console.log(2);
                        //handleReceiveAnswer(cont);
						localPeerConnection.setRemoteDescription(new RTCSessionDescription(cont.payload.sdp))
                        break;
...
function gotLocalMediaStream(mediaStream) {
  ...
  localPeerConnection = new RTCPeerConnection(iceConfig);
