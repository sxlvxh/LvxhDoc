...
artDialog.fn = artDialog.prototype = {
    ...
    constructor: function (config) {
       ...
       //added by huangwenhai on 2017-08-21 Fix:175324 对话框被ocx控件遮住 
        if(!(false==config.needPad)){
            var target;
		    target=this.dom.wrap;
		    if(0==$(".hy_dialog_pad",target).length){
                var iframeTop=0,iframeLeft=0,iframeWidth="100%",iframeHeight="100%";
		        ...
		        var $iframe=$("<iframe/>")
		    	//.css({padding:"12px",position:"absolute",zIndex:-1,width: "100%",height: "100%", top: iframeTop,left: iframeLeft,scrolling:"no",filter:"alpha(opacity=0);"})
                .css({position:"absolute",zIndex:-1,width: iframeWidth,height: iframeHeight, top: iframeTop,left: iframeLeft,scrolling:"no",filter:"alpha(opacity=0);"})
		    	.attr("frameborder","0")
		    	.addClass("hy_dialog_pad");
    ...
    */
    ...
    position: function () {
        ...
        var dom = this.dom,
	        ...
	        if( this.config.top)
        	{
	        	style.top = this.config.top + 'px';
        	}
	        else
        	{
	        	var topt = Math.max(parseInt(top), dt);
	        	if(topt < 30)
	        		{
	        		style.top = '0px';
	        		}
	        	...
	        	//style.top = ($($window).height() - this.config.height) / 2 + 'px';
        	}
	        if( this.config.left)
	    	{
	        	style.left = this.config.left + 'px';
	    	}
	        else
	    	{
	        	//style.left = Math.max(parseInt(left), dl) + 'px';
	        	var wwww = $($window).width();
	        	var twwww = this.config.width;
	        	if(twwww == "auto")
        		{
        		  twwww = 200;
        		}
	        	...
	        	else if(((wwww - twwww) / 2)<30)
	        	{
	        		style.left = '0px';
	        		}
	        	else
	        		{
	        		style.left = (wwww - twwww) / 2 + 'px';
	        		}
	    	}
...
//        };
        ...
        // 恢复焦点，照顾键盘操作的用户
        if (_activeElement) {
            try{_activeElement.focus();}catch(e){}
        }
