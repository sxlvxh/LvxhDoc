...
$(function(){
	...
	<!--../main/appLogin.action  mobilePhone  password-->
	$("#login_button").bind("click",function(){
		 // $("#form-login").submit();
		var opts = {
			url:"../main/appLogin.action",
			data:{"mobilePhone":$("#loginName").val(),"password":$("#password").val()},
			success:function(res){
			    if(res.code == 0){
					var json = JSON2.parse(res.obj.platConfig.platEnt.entParams);
					var optss = {
						"user": {
							"domainCode": res.obj.loing_user.domainCode,
							"name": res.obj.loing_user.name,
							"imgUrl": $.getImgUrl(res.obj.loing_user.imgUrl),
							"userCode": res.obj.loing_user.userCode
						},
						"sie":{
							"ip":json.extranet,
					...
					//csSdk.init(optss);
					setTimeout(function(){
			           location.href = "../main/page.action"; 
					},500)
			    }
		    }
		};
...
function detectVersion(){
	$._ajax({
		"url":"../platsoftlist/list.action",
		"data":{"softCode": "381de24ae80c458a856ebfd020443dfb"},
		"success": function(res){
			try{
				if(res.code == 0){
					if(res.result && res.result.length){
						var ver = res.result[0];
						var opt = {
								"versionCode":ver.version,
								"versionName":ver.version,
								"message":ver.description,
								"path":$.getImgUrl(ver.filePath),
								"isForcedUpdate":ver.mandatory
							};
						csSdk.detectVersion(opt);
					}else{
						$.hy_error(res,"未查询到版本信息 softCode： 381de24ae80c458a856ebfd020443dfb");
					}
				}
			}catch(e){alert(e)}
		}
...
<body class="login-layout blur-login">
	<div class="main-container">
	    <div class="main-content">
	         <img id="bgImg" class="me_icon" src="../theme/default/login_page/user-nocheck.png" style="height: 100%; width: 100%; left: 0px; z-index: -20; position: absolute;">
	         <div class="row">
	                                               ...
	                                               <form id="form-login" action='' method="post" style="margin: 0 0 10px 0;">
	                                                    <label class="block clearfix">
														    <span class="block input-icon input-icon-right">
														        <img id="user-img" src="../theme/default/login_page/user-nocheck.png" style ="position:absolute;left:85px;z-index:99;"/> 
														 ...
														 <label class="block clearfix">
														     <span class="block input-icon input-icon-right"> 
														        <img id="password-img" src="../theme/default/login_page/user-nocheck.png" style ="position:absolute;left:85px;z-index:99;"/> 
