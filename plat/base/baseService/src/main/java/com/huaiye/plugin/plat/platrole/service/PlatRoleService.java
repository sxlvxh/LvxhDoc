/*
 * . PlatRoleService.java
 * Copyright 2016. All Rights Reserved.
 */
package com.huaiye.plugin.plat.platrole.service;


import com.lvxh.plugin.platform.holder.BaseResult;
import com.lvxh.plugin.platform.service.BaseBusinessService;
import com.huaiye.plugin.plat.platrole.holder.PlatRoleHolder;

/**
 * . 角色表业务接口
 * 
 * @author 吕孝怀
 * @version V100R002C01-SNAPSHOT
 */
public interface PlatRoleService extends BaseBusinessService<PlatRoleHolder> {

	BaseResult setMenuRole(PlatRoleHolder holder);

}
