...
public class MsgDecoder {
	...
	private static final Logger LOG = Logger.getLogger(MsgDecoder.class);
	...
	public PackageData bytes2PackageData(byte[] data) {
		...
		if (checkSumInPkg != calculatedCheckSum) {
			LOG.debug("检验码不一致,msgid:" + msgHeader.getMsgId() + ",pkg:" + checkSumInPkg + ",calculated:"
					+ calculatedCheckSum);
		}
		return ret;
	}
	...
	private String parseBcdStringFromBytes(byte[] data, int startIndex, int lenth, String defaultVal) {
		try {
			...
			LOG.error("解析BCD(8421码)出错:" + e.getMessage(), e);
			e.printStackTrace();
			return defaultVal;
		}
	}
	...
	private int parseIntFromBytes(byte[] data, int startIndex, int length, int defaultVal) {
		try {
			...
			LOG.error("解析整数出错:" + e.getMessage(), e);
			e.printStackTrace();
			return defaultVal;
		}
	}
	...
	public LocationInfoUploadMsg toLocationInfoUploadMsg(PackageData packageData) {
		...
		ret.setAlarm(toTwo(ret.getWarningFlagField()));
		ret.setStatus(toTwo(ret.getStatusField()));
		// toTwo(ret.getStatusField());
	...
	private void customPaser(final byte[] data, int start, LocationInfoUploadMsg ret) {

		int id = this.parseIntFromBytes(data, start, 1);
		if (start + 1 > data.length) {
			return;
		}
		int length = this.parseIntFromBytes(data, start + 1, 1);
		if (length == 0) {
			length = 1;
		}
		...
		int d = this.parseIntFromBytes(data, start + 2, length);
		if (id == 37) {
			ret.setCustomData(toTwo(d));
			return;
		}
		System.out.println(id + " _ " + d);
		customPaser(data, start + 2 + length, ret);
		return;
	}

	public String toTwo(Integer I) {
	...
	private float parseFloatFromBytes(byte[] data, int startIndex, int length, float defaultVal) {
		try {
			...
			LOG.error("解析浮点数出错:" + e.getMessage(), e);
			e.printStackTrace();
			return defaultVal;
		}
	}
